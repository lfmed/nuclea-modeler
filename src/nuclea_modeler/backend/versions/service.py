...
def build_snapshot(sql: Sql, system_id: str) -> dict[str, Any]:
    ...
    if entity_ids:
        ids_in = ", ".join(f"'{_q(eid)}'" for eid in entity_ids)
        ...
        for r in attr_rows:
            attributes_by_entity.setdefault(r[1], []).append({
                ...
                "default_value": r[7],
                "is_primary_key": bool(r[8]) if r[8] is not None else False,
                "description_md": r[9],
                "business_rule": r[10],
                "sample_value": r[11],
    ...
    *,
    ...
    if make_active:
        ...
        delta.run(
            ...
            f"    updated_by = '{_q(actor)}' "
            f"WHERE system_id = '{_q(system_id)}' AND status = 'ACTIVE'",
        )
...
def restore_version(sql: Sql, version_id: str, actor: str) -> VersionOut:
    ...
    src = delta.fetch_one(
        ...
        f"WHERE version_id = '{_q(version_id)}'",
    )
    if not src:
...
def _get_version(sql: Sql, version_id: str) -> VersionOut:
    ...
    row = delta.fetch_one(
        ...
        WHERE v.version_id = '{_q(version_id)}'
        """,
    )
    if not row:
        raise HTTPException(404, f"version '{version_id}' not found")
